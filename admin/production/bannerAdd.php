...
//	echo '<pre>';
	...
	if ($id == 0) {
		...
		/* $rowAdmin = $db->row('SELECT fld_id,fld_name,fld_UserName,fld_UserPass,fld_countryId,fld_mobile,fld_status,userType FROM '.$table.' WHERE fld_id = :id AND `fld_status` != :status LIMIT 1', array('id' => $id, 'status'=>'-1' ));*/
		 $rowAdmin = $db->row('SELECT * FROM `__banner` WHERE  fld_id = '.$id.'');
		 //print_r($rowAdmin);
		 if(count($rowAdmin) == 0){
...
//echo 	$id = isset($rowAdmin['fld_id']) ? $rowAdmin['fld_id'] : ""; exit();
	...
	$fld_bannerState = isset($rowAdmin['fld_bannerState']) ? $rowAdmin['fld_bannerState'] : ""; 
	$fld_bannerCaption = isset($rowAdmin['fld_bannerCaption']) ? $rowAdmin['fld_bannerCaption'] : ""; 
	$fld_bannerImage = isset($rowAdmin['fld_bannerImage']) ? $rowAdmin['fld_bannerImage'] : ""; 
    $fld_bannerType = isset($rowAdmin['fld_bannerType']) ? $rowAdmin['fld_bannerType'] : "";
    $fld_bannerId = isset($rowAdmin['fld_bannerId']) ? $rowAdmin['fld_bannerId'] : "";
                        ...
                        <div class="box">
                            <div class="box-header with-border">
                                ...
                                <span class="box-controls pull-right" style="top:0.5rem">
                                    <a href="banner_list.php" class="btn btn-md btn-nblue btn-sm btn-block my-10">
                                        <!--<i class="ti-plus"></i>--> Back
                                    </a>
                                </span>
                            </div>
                            <div class="box-body">
                                <form action="bannerAdd2db.php" class="form-element" method="post" enctype="multipart/form-data">
                                    <div class="form-body">
                                        <!-- /row -->
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <?php 
														$rowCountryAdmin = $db->query('SELECT fld_state,fld_id FROM `__state` ');
													?>
                                                <div class="form-group">
                                                    <label class="text-info">State</label>
                                                    <select class="form-control select2" name="fld_bannerState" id="selectlang" required="" aria-invalid="false">
															<option value="">Select State</option>
															<?php 
																foreach($rowCountryAdmin as $key => $value){
															  ?>
																	<option value="<?=$value['fld_state'];?>" <?php if($fld_bannerState == $value['fld_state']){echo 'selected';}?>><?=$value['fld_state'];?></option>
															  <?php
																}
															  ?>
														</select>
                                                ...
                                                <div class="form-group">
                                                    <label class="text-info">Banner Images.<span class="text-danger">*</span>
                                                    ...
                                                    <input type="hidden" id="photo-doc" name="photo_doc" class="form-control pick-file" placeholder="Pick a File"  value="<?php echo $fld_bannerImage;?>"  required="required"  onKeyPress="return false;"><span class="uploader-button"></span>
<!--
...
-->
                                                    ...
                                                    <p id="photo"><?php if($fld_bannerImage!=''){ 
													echo '<a href="../../upload/banner/'.$fld_bannerImage.'?'.time().'" target="_blank" class="btn btn-primary btnfont">View Uploaded File</a>';}?></p>
                                                    
                                                </div>
                                                <div class="form-group">
                                                    <label class="text-info">Add caption<span class="text-danger">*</span></label>
                                                    <!-- Content Wrapper. Contains page content -->
                                                    <textarea rows="4" class="form-control" name="fld_bannerCaption" placeholder="Add Caption upto 50 Character"><?php echo $fld_bannerCaption;?></textarea>
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                <select  class="form-control select2" name="fld_bannerType" id="selecttype" required="">
                                                                    <option value="">Select Type</option>
                                                                    <option value="Product" <?php if($fld_bannerType == "Product"){echo 'selected';}?>>Product</option>
                                                                    <option value="Video" <?php if($fld_bannerType == "Video"){echo 'selected';}?>>Video</option>
                                                                    <option value="Noticeboard" <?php if($fld_bannerType == "Noticeboard"){echo 'selected';}?>>Noticeboard</option>
                                                                </select>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <div class="form-group">
                                                                ...
                                                                <select class="form-control select2" name="fld_bannerId" id="selectid" required="">
<!--                                                                    <option value="List products" <?php if($fld_bannerId == "List products"){echo 'selected';}?>>List products</option>-->
                                                                </select>
                                                            </div>
                                                        </div>
                                                    </div>
...
function loadOps(){
	...
	__initUploaders('photo-doc', 'jpg', 'JPEG', '2048kb', 'banner', users_id);
