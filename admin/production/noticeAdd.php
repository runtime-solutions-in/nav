...
//	echo '<pre>';
	...
	if ($id == 0) {
		...
		/* $rowAdmin = $db->row('SELECT fld_id,fld_name,fld_UserName,fld_UserPass,fld_countryId,fld_mobile,fld_status,userType FROM '.$table.' WHERE fld_id = :id AND `fld_status` != :status LIMIT 1', array('id' => $id, 'status'=>'-1' ));*/
		 $rowAdmin = $db->row('SELECT * FROM `__notice` WHERE  fld_id = '.$id.'');
		 //print_r($rowAdmin);
		 if(count($rowAdmin) == 0){
...
//echo 	$id = isset($rowAdmin['fld_id']) ? $rowAdmin['fld_id'] : ""; exit();
	...
	$fld_nState = isset($rowAdmin['fld_nState']) ? $rowAdmin['fld_nState'] : ""; 
	$fld_nName = isset($rowAdmin['fld_nName']) ? $rowAdmin['fld_nName'] : ""; 
	$fld_nThumb = isset($rowAdmin['fld_nThumb']) ? $rowAdmin['fld_nThumb'] : ""; 
    $fld_ndesc = isset($rowAdmin['fld_ndesc']) ? $rowAdmin['fld_ndesc'] : ""; 
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="d-flex align-items-center">
                    <div class="mr-auto">
                        <h3 class="page-title">Noticeboard</h3>
                    </div>
                </div>
            </div>
                        ...
                        <div class="box">
                            <div class="box-header with-border">
                                ...
                                <span class="box-controls pull-right" style="top:0.5rem">
                                    <a href="notice_list.php" class="btn btn-md btn-nblue btn-sm btn-block my-10">
                                        <!--<i class="ti-plus"></i>--> Back
                                    </a>
                                </span>
                            </div>
                            <div class="box-body">
                                <form action="noticeAdd2db.php" class="form-element" method="post" enctype="multipart/form-data">
                                    <div class="form-body">
                                        <!-- /row -->
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" name="fld_nState" id="selectlang" required="" aria-invalid="false">
															<option value="">Select State</option>
															<?php 
																foreach($rowCountryAdmin as $key => $value){
															  ?>
																	<option value="<?=$value['fld_state'];?>" <?php if($fld_nState == $value['fld_state']){echo 'selected';}?>><?=$value['fld_state'];?></option>
															  <?php
																}
															  ?>
														</select>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="fld_nName" class="form-control" placeholder="Notice Name" required="" value="<?php echo $fld_nName;?>" />
                                                </div>
                                                <div class="form-group">
                                                    <label class="text-info">Notice Thumbnail.<span class="text-danger">*</span>
                                                    ...
                                                    <input type="hidden" id="photo-doc" name="photo_doc" class="form-control pick-file" placeholder="Pick a File"  value="<?php echo $fld_nThumb;?>"  required="required"  onKeyPress="return false;"><span class="uploader-button"></span>
<!--
...
-->
                                                    ...
                                                    <p id="photo"><?php if($fld_nThumb!=''){ 
													echo '<a href="../../upload/event/'.$fld_nThumb.'?'.time().'" target="_blank" class="btn btn-primary btnfont">View Uploaded File</a>';}?></p>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="input-group input-group-sm">
                                                        <textarea rows="7" class="form-control" name="fld_ndesc" placeholder="Notice Description"><?php echo $fld_ndesc;?></textarea>
...
function loadOps(){
	...
	__initUploaders('photo-doc', 'jpg', 'JPEG', '2048kb', 'event', users_id);
