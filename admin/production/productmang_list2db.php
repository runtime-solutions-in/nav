<?php
   ...
   if($_SESSION['LOGIN_ROLE'] == 1){
	...
	if( empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
		if($_SESSION['LOGIN_ROLE'] == 1){
			$sql = "SELECT * FROM ".$table."  WHERE `fld_status` != '-1'";
		}else{
			$sql = "SELECT * FROM ".$table."  WHERE `fld_status` != '-1'";
		}
	...
	if (count($resadminUser) == 0) {	
		...
		foreach($resadminUser as $rowadminUser ){		
			...
			$action = '<a href="productmangAdd.php?id='.MyEncoder($rowadminUser['fld_id']).'" class="btn btn-icon-circle btn-nblue"><i class="fa fa-pencil"></i></a>';
		
			$action .= '&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="btn btn-icon-circle btn-danger delete" data-id="'.MyEncoder($rowadminUser['fld_id']).'" data-table="'.MyEncoder($table).'"  data-fld="'.MyEncoder('id').'" data-status="-1"  data-statusfld= "'.MyEncoder("fld_status").'" ><i class="fa fa-trash-o"></i></a>'; 
			
			//$thumbImage = "<img src='http://45.114.245.106/neevNav/upload/photo/".$rowadminUser['fld_productthumb']."'";
			$rowCountryAdmin = $db->row('SELECT fld_languageName,fld_id FROM `__language` WHERE fld_id = "'.$rowadminUser['selectlang'].'" ');
			 $rowProdAdmin = $db->row('SELECT fld_productname,fld_id FROM `__masterproduct` WHERE fld_id = "'.$rowadminUser['productid'].'" ');
			$nestedData[] = $rowadminUser['fld_id'];
			$nestedData[] = $rowCountryAdmin['fld_languageName'];
			$nestedData[] = $rowProdAdmin['fld_productname'];
			$nestedData[] = $rowadminUser['fld_uplodedby'];
