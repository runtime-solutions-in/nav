...
//	echo '<pre>';
	...
	if ($id == 0) {
		...
		/* $rowAdmin = $db->row('SELECT fld_id,fld_name,fld_UserName,fld_UserPass,fld_countryId,fld_mobile,fld_status,userType FROM '.$table.' WHERE fld_id = :id AND `fld_status` != :status LIMIT 1', array('id' => $id, 'status'=>'-1' ));*/
		 $rowAdmin = $db->row('SELECT * FROM `__business` WHERE  fld_id = '.$id.'');
		 //print_r($rowAdmin);
		 if(count($rowAdmin) == 0){
...
//echo 	$id = isset($rowAdmin['fld_id']) ? $rowAdmin['fld_id'] : ""; exit();
	...
	$fld_bLanguage = isset($rowAdmin['fld_bLanguage']) ? $rowAdmin['fld_bLanguage'] : ""; 
	$fld_bName = isset($rowAdmin['fld_bName']) ? $rowAdmin['fld_bName'] : ""; 
	$fld_bThumb = isset($rowAdmin['fld_bThumb']) ? $rowAdmin['fld_bThumb'] : ""; 
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="d-flex align-items-center">
                    <div class="mr-auto">
                        <h3 class="page-title">Business Opportunity</h3>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-12">
                        <div class="box">
                            <div class="box-header with-border">
                                <h4 class="box-title">Add Opportunity</h4>
                                <span class="box-controls pull-right" style="top:0.5rem">
                                    <a href="business_list.php" class="btn btn-md btn-nblue btn-sm btn-block my-10">
                                        <!--<i class="ti-plus"></i>--> Back
                                    </a>
                                </span>
                            </div>
                            <div class="box-body">
                                <form action="businessAdd2db.php" class="form-element" method="post" enctype="multipart/form-data">
                                    <div class="form-body">
                                        <!-- /row -->
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control select2" name="fld_bLanguage" id="selectlang" required="" aria-invalid="false">
															<option value="">Select Language</option>
															<?php 
																foreach($rowCountryAdmin as $key => $value){
															  ?>
																	<option value="<?=$value['fld_languageName'];?>" <?php if($fld_bLanguage == $value['fld_languageName']){echo 'selected';}?>><?=$value['fld_languageName'];?></option>
															  <?php
																}
															  ?>
														</select>
                                                ...
                                                <div class="form-group">
                                                    <label class="text-info">Business Opportunity Name</label>
                                                    <input type="text" name="fld_bName" class="form-control" placeholder="Business Opportunity Name" required="" value="<?php echo $fld_bName;?>" />
                                                </div>
                                                <div class="form-group">
                                                    <label class="text-info">Business Opportunity Thumbnail.<span class="text-danger">*</span>
...
-->
                                                    ...
                                                    <p id="photo"><?php if($fld_bThumb!=''){ 
													echo '<a href="../../upload/business/'.$photo_doc.'?'.time().'" target="_blank" class="btn btn-primary btnfont">View Uploaded File</a>';}?></p>
...
function loadOps(){
	...
	__initUploaders('photo-doc', 'jpg', 'JPEG', '2048kb', 'business', users_id);
