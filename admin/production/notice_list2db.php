   ...
   if($_SESSION['LOGIN_ROLE'] == 1){
	...
	if (count($resadminUser) == 0) {	
		...
		foreach($resadminUser as $rowadminUser ){		
			...
			$action = '<a href="noticeAdd.php?id='.MyEncoder($rowadminUser['fld_id']).'" class="btn btn-icon-circle btn-nblue"><i class="fa fa-pencil"></i></a>';
			...
			$nestedData[] = $rowadminUser['fld_nState'];
			$nestedData[] = $rowadminUser['fld_nName'];
			$nestedData[] = "<img src='http://45.114.245.106/neevNav/upload/event/".$rowadminUser['fld_nThumb']."'/>";
            $nestedData[] = $rowadminUser['fld_ndesc'];
