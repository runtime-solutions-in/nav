...
//	echo '<pre>';
	...
	if ($id == 0) {
		...
		/* $rowAdmin = $db->row('SELECT fld_id,fld_name,fld_UserName,fld_UserPass,fld_countryId,fld_mobile,fld_status,userType FROM '.$table.' WHERE fld_id = :id AND `fld_status` != :status LIMIT 1', array('id' => $id, 'status'=>'-1' ));*/
		 $rowAdmin = $db->row('SELECT * FROM `__videos` WHERE  fld_id = '.$id.'');
		 //print_r($rowAdmin);
		 if(count($rowAdmin) == 0){
...
//echo 	$id = isset($rowAdmin['fld_id']) ? $rowAdmin['fld_id'] : ""; exit();
	...
	$fld_videoType = isset($rowAdmin['fld_videoType']) ? $rowAdmin['fld_videoType'] : ""; 
	$fld_videoTitle = isset($rowAdmin['fld_videoTitle']) ? $rowAdmin['fld_videoTitle'] : ""; 
	$fld_videoThumb = isset($rowAdmin['fld_videoThumb']) ? $rowAdmin['fld_videoThumb'] : ""; 
    $fld_videoFile = isset($rowAdmin['fld_videoFile']) ? $rowAdmin['fld_videoFile'] : ""; 
    $fld_download = isset($rowAdmin['fld_download']) ? $rowAdmin['fld_download'] : ""; 
                        ...
                        <div class="box">
                            <div class="box-header with-border">
                                ...
                                <span class="box-controls pull-right" style="top:0.5rem">
                                    <a href="video_list.php" class="btn btn-md btn-nblue btn-sm btn-block my-10">
                                        <!--<i class="ti-plus"></i>--> Back
                                    </a>
                                </span>
                            </div>
                            <div class="box-body">
                                <form action="videoAdd2db.php" class="form-element" method="post" enctype="multipart/form-data">
                                    <div class="form-body">
                                        <!-- /row -->
                                        <div class="row">
                                            <div class="col-12">
                                                ...
                                                <div class="form-group">
                                                    <label class="text-info">Video Type <span class="text-danger">*</span></label>
                                                    <div class="controls">
                                                        <select name="fld_videoType" id="select" required="" class="form-control" aria-invalid="false">
                                                            <option>Select Category</option>
                                                            <option value="Product" <?php if($fld_videoType == "Product"){echo 'selected';}?>>Product</option>
                                                            <option value="TV Commercial" <?php if($fld_videoType == "TV Commercial"){echo 'selected';}?>>TV Commercial</option>
                                                            <option value="Others" <?php if($fld_videoType == "Others"){echo 'selected';}?>>Others</option>
                                                        </select>
                                                        <div class="help-block"></div>
                                                    </div>
                                                </div>
                                                <div class="form-group">
                                                    <label class="text-info">Video Title</label>
                                                    <input type="text" name="fld_videoTitle" class="form-control" placeholder="Video Name" required="" value="<?php echo $fld_videoTitle;?>" />
                                                </div>
                                                <div class="form-group">
                                                    <label class="text-info">Video Thumbnail <span class="text-danger">*</span>
                                                    ...
                                                    <input type="hidden" id="photo-doc" name="photo_doc" class="form-control pick-file" placeholder="Pick a File"  value="<?php echo $fld_videoThumb;?>"  required="required"  onKeyPress="return false;"><span class="uploader-button"></span>
<!--
...
-->
                                                    ...
                                                    <p id="photo"><?php if($fld_videoThumb!=''){ 
													echo '<a href="../../upload/videoThumb/'.$fld_videoThumb.'?'.time().'" target="_blank" class="btn btn-primary btnfont">View Uploaded File</a>';}?></p>
                                                    
                                                </div>
                                                
                                                 <div class="form-group">
                                                    <label class="text-info">Video File <span class="text-danger">*</span>
                                                    <span class="help-block text-danger">Maximum file size is 200MB.</span>
                                                    </label>
                                                    <div class="input-group input-group-sm">
                                                        <input type="file" name="photo_big" id="upload-photo_big-doc" accept="application/pdf" class="form-control hidden pick-file-target" >
                                                    
                                                    <input type="hidden" id="photo_big-doc" name="photo_big_doc" class="form-control pick-file" placeholder="Pick a File"  value="<?php echo $fld_videoFile;?>"  required="required"  onKeyPress="return false;"><span class="uploader-button"></span>
<!--
                                                        <span class="input-group-btn">
                                                            <button type="button" class="btn btn-default fa fa-upload">
                                                                Upload</button>
...
-->
                                                    ...
                                                    <p id="photo_big"><?php if($fld_videoFile!=''){ 
													echo '<a href="../../upload/videoFile/'.$fld_videoFile.'?'.time().'" target="_blank" class="btn btn-primary btnfont">View Uploaded File</a>';}?></p>
                                                </div>
                                                
                                                <div class="form-group demo-radio-button">
                                                    <label class="text-info">Downloadable
                                                       <span class="text-danger">*</span>
                                                    </label>
                                                    <input name="fld_download" type="radio" value="Yes" id="radio_12" class="radio-col-blue" <?php if($fld_download == "Yes") {echo "checked";}?>>
                                                        <label for="radio_12">Yes</label>
                                                    <input name="fld_download" type="radio" value="No" id="radio_7" class="radio-col-red" <?php if($fld_download == "No") {echo "checked";}?>>
...
function loadOps(){
	...
	__initUploaders('photo-doc', 'jpg', 'JPEG', '2048kb', 'videoThumb', users_id);
	__initUploaders('photo_big-doc', 'jpg', 'JPEG', '2048kb', 'videoFile', users_id);
