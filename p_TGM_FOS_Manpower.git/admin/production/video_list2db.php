   ...
   if($_SESSION['LOGIN_ROLE'] == 1){
	...
	if (count($resadminUser) == 0) {	
		...
		foreach($resadminUser as $rowadminUser ){		
			...
			$action = '<a href="videoAdd.php?id='.MyEncoder($rowadminUser['fld_id']).'" class="btn btn-icon-circle btn-nblue"><i class="fa fa-pencil"></i></a>';
			...
			$nestedData[] = $rowadminUser['fld_videoType'];
			$nestedData[] = $rowadminUser['fld_videoTitle'];
			$nestedData[] = "<img src='http://45.114.245.106/neevNav/upload/videoThumb/".$rowadminUser['fld_videoThumb']."'/>";
            $nestedData[] = "<img src='http://45.114.245.106/neevNav/upload/videoFile/".$rowadminUser['fld_videoFile']."'/>";
            $nestedData[] = $rowadminUser['fld_download'];
