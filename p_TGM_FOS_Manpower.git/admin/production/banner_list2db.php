   ...
   if($_SESSION['LOGIN_ROLE'] == 1){
	...
	if (count($resadminUser) == 0) {	
		...
		foreach($resadminUser as $rowadminUser ){		
			...
			$action = '<a href="bannerAdd.php?id='.MyEncoder($rowadminUser['fld_id']).'" class="btn btn-icon-circle btn-nblue"><i class="fa fa-pencil"></i></a>';
			...
			$nestedData[] = $rowadminUser['fld_bannerState'];
			$nestedData[] = "<img src='http://45.114.245.106/neevNav/upload/banner/".$rowadminUser['fld_bannerImage']."'/>";
            $nestedData[] = $rowadminUser['fld_bannerCaption'];
            $nestedData[] = $rowadminUser['fld_bannerType'];
			$nestedData[] = $rowadminUser['fld_bannerId'];
