   ...
   if($_SESSION['LOGIN_ROLE'] == 1){
	...
	if (count($resadminUser) == 0) {	
		...
		foreach($resadminUser as $rowadminUser ){		
			...
			$action = '<a href="vasAdd.php?id='.MyEncoder($rowadminUser['fld_id']).'" class="btn btn-icon-circle btn-nblue"><i class="fa fa-pencil"></i></a>';
			...
			$nestedData[] = $rowadminUser['fld_sLanguage'];
			$nestedData[] = $rowadminUser['fld_sName'];
			$nestedData[] = "<img src='http://45.114.245.106/neevNav/upload/vas/".$rowadminUser['fld_sThumb']."'/>";
            $nestedData[] = $rowadminUser['fld_sdesc'];
